...
$g(n)$ is the cost accumulated so far. $h(n)$ is the heuristic, your estimate of remaining distance to $t$. Dijkstra is exactly A* with $h \equiv 0$.

---

## 3. The Heuristic Function $h$

$h: V \to \mathbb{R}^+$ maps each node to an estimate of the remaining cost to reach the target $t$.
...
- Euclidean: yes. Straight-line distance is always $\le$ actual grid path distance.
- Manhattan: yes. On a 4-connected grid (up/down/left/right only), the Manhattan distance is exactly the minimum number of steps ignoring obstacles. With obstacles in the way, the true cost can only be $\ge$ Manhattan, so it never overestimates.
...
**Proof by contradiction:** Suppose A* terminates with a suboptimal path of cost $l' > l^*$. Let $v_0, v_1, \ldots, v_k = t$ be the optimal path of cost $l^*$. Since A* did not follow this path, some node $v_i$ on it must still be sitting in the open set when $t$ was popped.

The priority of $v_i$ is $g(v_i) + h(v_i)$. By admissibility, $h(v_i) \le$ cost from $v_i$ to $t$ along the optimal path. So:

$$f(v_i) = g(v_i) + h(v_i) \le l^*$$

But A* popped $t$ with $f(t) = g(t) = l'$, and the priority queue always pops the minimum. So $l' \le f(v_i) \le l^*$, which contradicts $l' > l^*$. $\square$
...
When $h \equiv 0$, A* reduces exactly to Dijkstra. As $h$ grows closer to $h^*$, A* explores fewer and fewer nodes. If $h = h^*$ exactly, A* explores only the nodes on the optimal path, nothing else.
...
Euclidean assumes you can travel diagonally, so it underestimates more aggressively. More nodes end up passing the $f(n) \le f^*$ threshold and get explored. Manhattan "knows" the actual movement model and prunes harder.

Expected ranking on this assignment's grids:

$$\text{nodes explored: } \text{Dijkstra} > \text{A*\_euclidean} > \text{A*\_manhattan}$$
...
Worst case matches Dijkstra because with $h \equiv 0$ or a very poor heuristic, A* is Dijkstra. The heuristic improves practical performance but does not change the asymptotic worst case.
